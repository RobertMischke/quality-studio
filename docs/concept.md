...
A repository may curate review scope in `.quality/scope.json`, validated by
`schemas/scope.v1.schema.json`. Its `rules` array is evaluated in declaration
order against canonical repository-relative paths (`/` separators, ordinal case
comparison). Patterns use Git wildmatch syntax: `*` and `?` do not cross `/`,
`**` does, bracket classes are supported, a leading `/` anchors at the repository
root, and a trailing `/` targets a directory and its descendants. For example:

```json
{
  "$schema": "https://agent-orchestrator.dev/quality/schemas/scope.v1.schema.json",
  "rules": [
    {
      "action": "exclude",
    ...
    {
      "action": "include",
      "pattern": "src/Interop/CheckedIn.generated.cs"
    }
  ]
}
```

Precedence is deterministic. Quality Studio first applies built-in exclusions
for `.quality`, review-meta sidecars, `bin`/`obj`, package caches, conventional
generated-source names, and the `<auto-generated>` source marker. It then applies
repository `.gitignore` files from the root toward the candidate (including Git
negation rules), followed by `.quality/scope.json`. Within each source, the last
matching rule wins. Therefore a curated `include` can opt a required checked-in
generated file back into review, while a curated `exclude` can add project-specific
test or fixture policy. Every curated `exclude` requires a human-readable reason.

Excluded candidates never become File units or aggregate members. The tree API
returns them in each affected aggregate node's `excluded` array, ordered by path
and reason. Aggregate reviews store the same entries in `aggregate.excluded` and
include them in the aggregate-members digest, so changing an effective rule or
reason deterministically makes the affected stored aggregate stale.
