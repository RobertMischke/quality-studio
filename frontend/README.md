...
The development server runs at `http://localhost:4200` and proxies `/api` to the QS API at `http://127.0.0.1:5127`. If the API is unavailable, the shell shows clearly labeled preview data so the workspace remains inspectable.
